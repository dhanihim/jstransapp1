class CustomerLocationsController < ApplicationController
  ...
  def show
    ...
    pricelist = CustomerLocationPricelist.where("customer_location_id = ? and active = 1", params[:id])

    pricelist.each do |pricelist|
      customer_location_id = pricelist.customer_location_id
      location_id = pricelist.location_id
